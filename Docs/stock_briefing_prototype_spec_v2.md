...
| **Market-data tool** (thin interface; offline JSON-fixture default; key-gated **Alpha Vantage** live adapter, ticker-agnostic — **MSFT live example, MRB offline example**) | `design.md §12a` → `app/tools/market_data.py` | ✅ **done** (7 tests) |
| **Orchestrator → retriever + market_data → specialist → gate → synthesizer** (LangGraph; synthesizer on the pass edge only) | `app/orchestrator.py` + `app/agents/*` + `app/eval/gate.py` | ✅ **T3 done** (stub gate / keyword retriever) |
...
**Done:** pack v2.2 · corpus + quote fixture · golden 20 rows (**CLEAN**, both verticals) · `design.md §12/§12a/§12c` (rev10) · **T0** (models incl. `Quote`/`RunTrace` + config) · **T1** (Munich-Re-styled dual-surface UI: welcome → advisor briefing → ⚙ "Show my work"; advisor-only login) · **T2** (policy loader) · **T3** (LangGraph orchestrator → retriever + market_data → specialist → gate → synthesizer; **synthesizer reachable only on the gate's pass edge** — `test_synthesizer_unreachable_on_fail`) · **T4b** (market-data tool with the **live MSFT** Alpha Vantage adapter + offline fixture fallback). **17 tests, ruff + mypy --strict clean.**
**Not built yet:** **T4** real retriever (embeddings + Chroma; currently a keyword stub) → T5 guardrails → T6 full gate (support + rubric cascade; currently a deterministic-floor stub) → T7 entitlement signature → T9 audit → T10 golden harness. **UI** still calls the canned stub backend; wiring "Show my work" to the real graph trace is the immediate next step.
...
- **What:** the advisor surface is currently a **Munich-Re-styled Streamlit** app (navy + turquoise theme, branded verdict banners, quote stat card, turquoise citation cards; the advisor login is just Dana, and a **gear icon** opens the "Show my work" glass box). That hits the rubric's equal-weight UI bar at ~80% brand fidelity, fast and offline. For a production-grade, pixel-perfect on-brand experience, the next step is a **React/Next + Tailwind** frontend over a **thin FastAPI wrapper** on the existing backend.
